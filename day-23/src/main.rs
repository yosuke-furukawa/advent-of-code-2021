...
impl Map {
    ...
    fn lowest_cost(&self) -> i64 {
        ...
        let mut stack: Vec<StepInfo> = vec![StepInfo {
            map: self.clone(),
        ...
        while !stack.is_empty() {
            ...
            if step.cost > lowest_cost || step.moves.is_empty() {
                stack.pop();
                continue;
            }
    ...
    fn plot(piece: usize) -> char {
        if piece == 0 {
            return '.';
        }
        (b'A' + (piece - 1) as u8) as char
    }
    fn debug(&self) {
        println!("#############");
        print!("#");
        for i in 0..11 {
            print!("{}", Self::plot(self.open_space[i]));
        }
        println!("#");
        println!(
            "###{}#{}#{}#{}###",
            Self::plot(self.rooms[0][0]),
            Self::plot(self.rooms[1][0]),
            Self::plot(self.rooms[2][0]),
            Self::plot(self.rooms[3][0])
        );
        for i in 1..self.rooms[0].len() {
            println!(
                "  #{}#{}#{}#{}#",
                Self::plot(self.rooms[0][i]),
                Self::plot(self.rooms[1][i]),
                Self::plot(self.rooms[2][i]),
                Self::plot(self.rooms[3][i])
            );
        }
        println!("  #########");
    }
    fn unfolded(&self) -> Map {
        let mut map = Map::new();
        map.open_space = self.open_space;
        map.rooms = Vec::new();
        map.rooms
...
fn part2(arg: &str) -> i64 {
    let map = parse(arg).unfolded();
    map.lowest_cost()
}

fn main() {
    let map = parse("./data.txt");
    map.debug();
    println!("{}", part1("./data.txt"));
    let map = parse("./data.txt").unfolded();
    map.debug();
